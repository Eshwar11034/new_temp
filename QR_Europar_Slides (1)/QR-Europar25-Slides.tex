...
\begin{document}
	...
	\input{Motivation}
	\input{Challenge}
	\input{Questions-Contributions}
	
	% \input{Slide_Intro_NLP.tex}
	% \input{Slide_Motivation_SLSQP.tex}
	% \input{Slide_Bridge_to_QR.tex}
	% \input{Slide_QR_Theory.tex}
	\input{SequentialAlgo.tex}   % The slide with Algorithm 2
	\input{Kernels.tex}        % The slide visualizing the kernels
	
	% --- Placeholders for the rest of the presentation ---
	\input{NaiveBarrier.tex}
	\input{QrAsDag.tex} % FIXED: Added missing .tex extension (was QrAsDag in original)
	\input{BarrierinDAG.tex} % FIXED: Capitalization to match file name
	\input{TwoQueueIntro.tex}
	\input{TwoQueueSim_A.tex}
	\input{TwoQueueSim_B.tex}
	\input{TwoQueueSim_C.tex}
	\input{Results.tex}
	\input{DefineAlphaAndBeta}
	\input{TuningResultsHeatmaps}
	\input{TuningResultsBargraph}
	\input{ScalingWithDataSize}
	\input{ScalingWithThreads}
	\input{SLSQP}
	\input{Plasma}
